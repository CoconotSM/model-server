...
def getKeyword(text, model_num):
  input_ids = tokenizer.encode(text)
  input_ids = [tokenizer.bos_token_id] + input_ids + [tokenizer.eos_token_id]
  input_ids = torch.tensor([input_ids])

  summary_text_ids = modelSET[model_num].generate(
    input_ids=input_ids,
...
def final_convert(script_data, input_text, model_data):
    target_image = Image.open('static/background_img'+model_data+'.png')          # 배경이미지 : 클라이언트에서 넘어오는 버튼의 인덱스 값을 받아서, 배경 이미지 오픈
    ...
    keyword = getKeyword(script_data, model_num) 
    # title = getTitle
...
def final_convert(script_data, input_text, model_data):
    target_image_num = model_data+1
    target_image = Image.open('static/background_img'+target_image_num+'.png')          # 배경이미지 : 클라이언트에서 넘어오는 버튼의 인덱스 값을 받아서, 배경 이미지 오픈
                                                                     # 배경이미지 및  위치 배정 함수를 새로 만들어야 함.
    print("-------"+target_image)
    print(target_image_num)
    # 모델에 따라 다른 배치 구성하기
    # if model_data == 0 :
    #   width, height = target_image.width, target_image.height  # (1966, 1102)
    #   margin = width * 0.04
